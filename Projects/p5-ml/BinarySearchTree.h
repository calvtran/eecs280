...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int size_impl(const Node *node) {
      ...
      count += size_impl(node->left);
      count += size_impl(node->right);
      return count;
  }
  ...
  static int height_impl(const Node *node) {
      ...
      if (!node)
          return 0;
      left += height_impl(node->left);
      right += height_impl(node->right);
      if (left > right)
          return left;
      else
          return right;
      ...
      else
      {
          if (node->left != nullptr)
          {
              if (less(node->left->datum, node->datum))
                  left = check_sorting_invariant_impl(node->left, less);
              else
                  return false;
          }
          if (node->right != nullptr)
          {
              if (less(node->datum, node->right->datum))
                  right = check_sorting_invariant_impl(node->right, less);
              else
                  return false;
          }
      }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
      if (!node)
          return nullptr;
      else if (!less(val,node->datum)&&!less(node->datum,val))
          return  min_greater_than_impl(node->right, val, less);
      else if (less(val, node->datum))
          if (!node->left)
              return node;
          else if (less(node->left->datum, val))
              return node;
          else if (!less(node->left->datum,val)&&!less(val,node->left->datum)
                    && !node->left->right)
              return node;
          else
