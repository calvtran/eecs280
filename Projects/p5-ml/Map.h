...
public:
  ...
  Iterator find(const Key_type& k) const
  {
      Pair_type me;
      me.first = k;
      //me.second = Value_type();
      if (entries.find(me) != end())
          return entries.find(me);
      else
          return entries.end();
  }
  ...
  {
      std::pair<Iterator, bool> me;
      if (entries.find(val) == end())
      {
          entries.insert(val);
          me.first = entries.find(val);
          me.second = true;
          return me;
      }
      else
